...
class MPullRefreshScope extends StatefulWidget {
  const MPullRefreshScope({Key? key, this.child}) : super(key: key);

  final Widget? child;

  @override
  State<MPullRefreshScope> createState() => _MPullRefreshScopeState();
}

class _MPullRefreshScopeState extends State<MPullRefreshScope> {
...
class MSliverPullRefreshIndicator extends StatefulWidget {
  ...
  /// a refresh.
  const MSliverPullRefreshIndicator({
    Key? key,
  ...
  static Widget buildRefreshIndicator(
    ...
    if (refreshState == RefreshIndicatorMode.refresh) {
      ...
      widget = Opacity(
        opacity: pulledExtent >= 80 ? 1 : (pulledExtent / 80),
  ...
  @override
  MSliverPullRefreshIndicatorState createState() =>
      MSliverPullRefreshIndicatorState();
}

class MSliverPullRefreshIndicatorState
    extends State<MSliverPullRefreshIndicator> {
  double _height = 0;
  ...
  void initState() {
    ...
    var state = context.findAncestorStateOfType<_MPullRefreshScopeState>();
    assert(
      state != null,
